...
<html lang="en">
	<head>
		...
		<title>Retrieval | Information Retrieval Project</title>
		...
		<main class="wrapper">
			...
			<nav class="navigation">
				<section class="container">
					...
					<ul class="navigation-list float-right">
						...
						<li class="navigation-item">
							<a class="navigation-link active" href="/irp/retrieval">Retrieval</a>
						</li>
						<li class="navigation-item">
			...
			<section class="container">
				...
				<?php if ($keyword): ?>
					<div id="app">
						<p>
							Pencarian untuk kata kunci <strong><?=$keyword?></strong> memerlukan waktu {{ time }} detik.
						</p>
						<table>
						<tbody>
							<tr v-for="item in list">
								<td>
									<a :href="item.url"><strong>{{ item.title }}</strong></a> <em>({{ item.similarity }})</em>
									<p style="margin-bottom:0;">{{ item.content }} <em><a :href="item.url">[...]</a></em></p>
								</td>
							</tr>
						</tbody>
						</table>
				...
				onInfinite: function () {
					this.$http.get(api, {
						...
						if (res.data.docs.length > 0) {
							this.list = this.list.concat(res.data.docs);
